...
public class AssignNewDev {

    private JFrame frame;
    private ArrayList<ButtonItem> dev_buttons = new ArrayList<ButtonItem>();
    private JList buttonlist_visual;
    private JScrollPane scrollPane;
    private ArrayList<DevelopmentEmployee> devsNotAdded = new ArrayList<DevelopmentEmployee>();
    private ActivityPage activityPage;

    public AssignNewDev(ActivityPage activityPage) {
        for (DevelopmentEmployee dev : activityPage.getDevsInProject()){
            if (!activityPage.getDevsInActivity().contains(dev)){
                this.devsNotAdded.add(dev);
            }
        }
    ...
    public void setup(){
        for(DevelopmentEmployee dev : this.devsNotAdded){
            ButtonItem button = new ButtonItem(this.activityPage, dev, dev.getInitials(), "AssignNewDev");
